...
${EXTERNTERMINAL} ${MEMCHECK} ${CLIENT} -f ${SOCKET} ${TIME} -p -W ${WRITEFILESDIR}/file1.txt,${WRITEFILESDIR}/file2.txt -r ${WRITEFILESDIR}/file1.txt -c ${WRITEFILESDIR}/file2.txt &
echo
echo "AVVIO CLIENT 2 TESTO -w CON FILE ILLIMITATI E -D PER I FILE ESPULSI"
${EXTERNTERMINAL} ${MEMCHECK} ${CLIENT} -f ${SOCKET} ${TIME} -p -w ${WRITEFILESDIR} -D ${EXPELLED} &
echo
echo "AVVIO CLIENT 3 TESTO -w CON 4 FILE AL MASSIMO, E -R CON -d PER SALVARE I FILE IN ${SAVEDIR}"
${EXTERNTERMINAL} ${MEMCHECK} ${CLIENT} -f ${SOCKET} ${TIME} -p -w ${WRITEFILESDIR},4 -R -d ${SAVEDIR} &
echo
echo "AVVIO CLIENT 3 TESTO -l E -u DI 2 FILE, E -R"
${EXTERNTERMINAL} ${MEMCHECK} ${CLIENT} -f ${SOCKET} ${TIME} -p -l ${WRITEFILESDIR}/file1.txt,${WRITEFILESDIR}/file2.txt -u ${WRITEFILESDIR}/file1.txt,${WRITEFILESDIR}/file2.txt -R &
